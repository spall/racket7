...
;; What a futures continuation looks like
...
(define/who (maybe-block tag)
  (when (future? (current-future)) ;; running in a future
	(check who continuation-prompt-tag? tag)
	(let ([fp (strip-impersonator (future*-prompt (current-future)))]
...
;; This could block because prompt tag could be past future tag
...
;; TODO. maybe this is handled elsewhere?
...
;; TODO: could this ever need to block? 
